...
func (r *TaskRepo) GetByID(ctx context.Context, id string) (*entity.Task, error) {
	...
	err := scanRow(row, &t.ID, &t.Type, &status, &t.MachineID, &t.Payload, &t.Result, &t.Error, &t.Logs, &t.CreatedAt, &t.UpdatedAt)
	if err != nil {
...
func (r *TaskRepo) List(ctx context.Context, limit, offset int) ([]entity.Task, error) {
	...
	for scanner.Next() {
		var t entity.Task
		var status string
		err := scanRow(scanner, &t.ID, &t.Type, &status, &t.MachineID, &t.Payload, &t.Result, &t.Error, &t.Logs, &t.CreatedAt, &t.UpdatedAt)
		if err != nil {
