...
func newUseCaseForTest(client dockerclient.Client, netRepo repo.NetworkRepo, rmRepo repo.RemoteMachineRepo) *NetworkUseCase {
...
func TestListNetworks_EmptyCache_FetchesFromDocker(t *testing.T) {
	...
	mockCli := &mockDockerClient{
		networkListFn: func(_ context.Context) ([]entity.Network, error) {
			...
			return []entity.Network{
				{ID: "abc123", Name: "my-bridge", Driver: "bridge"},
			}, nil
		},
	}
	...
	mockNetRepo := &mockNetworkRepo{
		...
		upsertBatchFn: func(_ context.Context, _ string, networks []entity.Network) error {
			...
			require.Equal(t, "my-bridge", networks[0].Name)
			return nil
		},
	}
...
func TestListNetworks_CacheHit_ReturnsCached(t *testing.T) {
	...
	cachedNetworks := []entity.Network{
		{ID: "cached-1", Name: "cached-net", Driver: "bridge"},
	}
...
func TestCreateNetwork(t *testing.T) {
	...
	mockCli := &mockDockerClient{
		networkCreateFn: func(_ context.Context, name, driver string) (*entity.Network, error) {
			...
			require.Equal(t, "bridge", driver)
			return &entity.Network{ID: "new-id", Name: "test-net", Driver: "bridge"}, nil
		},
	}
	...
	network, err := uc.CreateNetwork(context.Background(), "local", "test-net", "bridge")
	require.NoError(t, err)
...
func TestDeleteNetwork_RejectsBuiltIn(t *testing.T) {
	dockerCalled := false
	...
	for _, name := range builtIns {
		...
		require.Error(t, err)
		require.Contains(t, err.Error(), "cannot delete built-in network")
	}

	require.False(t, dockerCalled, "expected Docker NOT to be called for built-in networks")
}

func TestDeleteNetwork_SucceedsOnUserNetwork(t *testing.T) {
	deleteCalled := false
	cacheInvalidated := false
	...
	mockCli := &mockDockerClient{
		networkDeleteFn: func(_ context.Context, networkID string) error {
			deleteCalled = true
			require.Equal(t, "user-net", networkID)
			return nil
		},
	}
	...
	err := uc.DeleteNetwork(context.Background(), "local", "user-net")
	require.NoError(t, err)
...
func TestDeleteNetwork_DockerError(t *testing.T) {
	mockCli := &mockDockerClient{
		networkDeleteFn: func(_ context.Context, _ string) error {
			return errors.ErrNetworkNotFound
		},
	}

	uc := newUseCaseForTest(mockCli, &mockNetworkRepo{}, &mockRemoteMachineRepo{})

	err := uc.DeleteNetwork(context.Background(), "local", "non-existent-net")
	require.Error(t, err)
	require.ErrorIs(t, err, errors.ErrNetworkNotFound)
}
...
func TestInspectNetwork(t *testing.T) {
	...
	mockCli := &mockDockerClient{
		networkInspectFn: func(_ context.Context, networkID string) (*entity.Network, error) {
			...
			return &entity.Network{ID: "inspect-id", Name: "my-net", Driver: "overlay"}, nil
		},
	}
	...
	require.Equal(t, "overlay", network.Driver)
